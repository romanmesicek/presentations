<script setup>
/**
 * ClosingSlide — Thank you + QR in the Rail + Wand vocabulary.
 * Props: photo, speaker, affiliation, email, linkedin, website, github,
 *        qr, ctaHeadline, ctaTitle, ctaBody, ctaUrl.
 */
defineProps({
  photo: { type: String, default: '' },
  speaker: { type: String, default: '' },
  affiliation: { type: String, default: '' },
  email: { type: String, default: '' },
  linkedin: { type: String, default: '' },
  website: { type: String, default: '' },
  github: { type: String, default: '' },
  qr: { type: String, default: '' },
  ctaHeadline: { type: String, default: 'Vielen Dank.' },
  ctaTitle: { type: String, default: '' },
  ctaBody: { type: String, default: '' },
  ctaUrl: { type: String, default: '' },
})

const base = import.meta.env.BASE_URL
function asset(p) {
  if (!p) return ''
  if (/^(https?:|data:|mailto:)/.test(p)) return p
  return p.startsWith('/') ? base + p.slice(1) : p
}
</script>

<template>
  <div class="closing">
    <aside class="rail">
      <div class="top">
        <img v-if="photo" :src="asset(photo)" :alt="speaker" class="photo" />
        <div class="speaker">{{ speaker }}</div>
        <div class="aff" v-if="affiliation">{{ affiliation }}</div>
      </div>
      <div class="socials">
        <a v-if="email" :href="`mailto:${email}`" aria-label="E-Mail">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="0"/><polyline points="22,4 12,13 2,4"/></svg>
        </a>
        <a v-if="linkedin" :href="linkedin" target="_blank" aria-label="LinkedIn">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
        </a>
        <a v-if="website" :href="website" target="_blank" aria-label="Website">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
        </a>
        <a v-if="github" :href="github" target="_blank" aria-label="GitHub">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/></svg>
        </a>
      </div>
    </aside>

    <section class="body">
      <h1>{{ ctaHeadline }}</h1>
      <div class="cta">
        <div class="cta-text">
          <div class="cta-title">{{ ctaTitle }}</div>
          <p class="cta-body" v-if="ctaBody">{{ ctaBody }}</p>
          <a v-if="ctaUrl" :href="ctaUrl" target="_blank" class="cta-url">{{ ctaUrl.replace(/^https?:\/\//, '') }}</a>
        </div>
        <img v-if="qr" :src="asset(qr)" class="qr" alt="QR-Code" />
      </div>
    </section>
  </div>
</template>

<style scoped>
.closing {
  position: absolute;
  inset: 0;
  display: grid;
  grid-template-columns: 11rem 1fr;
  color: #e5e7eb;
  font-family: 'Raleway', sans-serif;
}
.rail {
  padding: 1.75rem 1.25rem;
  display: flex;
  flex-direction: column;
  justify-content: space-between;
  border-left: 3px solid #818cf8;
}
.photo {
  width: 3.5rem;
  height: 3.5rem;
  object-fit: cover;
  margin-bottom: 0.75rem;
}
.speaker {
  font-size: 1rem;
  font-weight: 700;
  letter-spacing: -0.01em;
  color: #ffffff;
}
.aff {
  font-size: 0.75rem;
  color: rgba(255, 255, 255, 0.7);
  margin-top: 0.15rem;
}
.socials {
  display: flex;
  gap: 0.65rem;
}
.socials a {
  color: #a5b4fc;
  opacity: 0.85;
  transition: opacity 0.15s, color 0.15s;
  display: inline-flex;
}
.socials a:hover { opacity: 1; color: #ffffff !important; }
.body {
  padding: 2.5rem 3rem;
  display: flex;
  flex-direction: column;
  justify-content: space-between;
}
h1 {
  font-size: 3.5rem;
  line-height: 1;
  font-weight: 700;
  letter-spacing: -0.03em;
  margin: 0;
  border-bottom: none;
}
.cta {
  display: grid;
  grid-template-columns: 1fr auto;
  gap: 2rem;
  align-items: end;
  padding-top: 1.25rem;
  border-top: 1px solid rgba(255, 255, 255, 0.08);
}
.cta-title {
  font-size: 1.4rem;
  font-weight: 700;
  letter-spacing: -0.01em;
}
.cta-body {
  margin-top: 0.35rem;
  font-size: 0.9rem;
  color: rgba(255, 255, 255, 0.7);
  line-height: 1.4;
  max-width: 36ch;
}
.cta-url {
  display: inline-block;
  margin-top: 0.75rem;
  font-family: 'JetBrains Mono', ui-monospace, monospace;
  font-size: 0.7rem;
  color: #a5b4fc !important;
  letter-spacing: 0.05em;
}
.qr {
  width: 7rem;
  height: 7rem;
}
</style>
